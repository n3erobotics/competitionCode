...
extern VideoCapture webcamL, webcamR;
...
void check_crossroad(){
	...
	// ONly makes sense if we detected 4 objects already
	if(no_objects>4){
...
//			imshow("Draw", drawing);
		...
		while(!first_line){
			...
			int N=objects.size()-1;
			for( size_t i = 0; i< objects.size(); i++ ){
				if(objects.at(N-i).at(Y)>50 && objects.at(N-i).at(X)>200){
...
void change_lane(bool direction){
	...
	if(!direction){
		if(present_lane == OUTSIDE){
			...
			message.str("");
			message << "sl" << CHANGE_2LEFT_LANE_ANGLE;
			serialPort.sendArray(message.str(), message.tellp());
		}else{
			...
			putText(drawing, "CHANGING", Point2f(350,250), FONT_HERSHEY_PLAIN, 2, Scalar(255,255,0), 2);
			message.str("");
			message << "sr" << CHANGE_2RIGHT_LANE_ANGLE;
			serialPort.sendArray(message.str(), message.tellp());
		}
	}else{
		if(present_lane == OUTSIDE){
			...
			putText(drawing, "CHANGING", Point2f(350,250), FONT_HERSHEY_PLAIN, 2, Scalar(255,255,0), 2);
			message.str("");
			message << "sr" << CHANGE_2LEFT_LANE_ANGLE;
			serialPort.sendArray(message.str(), message.tellp());
		}else{
			// If we are on left and we discover an object on right we follow it
			for(size_t i=0; i<objects.size(); i++){
...
void simple_distance_lines(vector<double> lane, bool direction){
	int distance, teta, middle;
	char dir;

	middle = MID_LINE_DISTANCE + (lane.at(Y)-230);
	distance=lane.at(X)-(width/2);
	if(direction){
		if(distance > middle){
			dir='l';
			teta=(int) ( (float)(distance-middle)/TURN_COEFICIENT);
		}else{
			dir='r';
			teta= (int) ( (float)(middle -distance)/TURN_COEFICIENT);
		}
	}else{
		if(distance > middle){
			dir='r';
	...
	message.str("");
	message << "s" << dir << teta;
	putText(drawing, message.str(), Point2f(150,250), FONT_HERSHEY_PLAIN, 2, Scalar(255,255,0), 2);
	serialPort.sendArray(message.str(), message.tellp());
...
int get_line2follow(){
	...
	message.str("");
	message << line2follow.at(AREA);
	putText(drawing, message.str(), Point2f(150,450), FONT_HERSHEY_PLAIN, 2, Scalar(255,255,0), 2);
...
void *trackHandler(void*){
	...
	if(!webcamR.open(DEVICE_RIGHT)){
		cout << "ERROR: Couldn't open RIGHT webcam" << endl;
		pthread_exit(NULL);
	}
	webcamR.set(CV_CAP_PROP_FRAME_WIDTH, width);
	webcamR.set(CV_CAP_PROP_FRAME_HEIGHT, height);
	//	webcamR.set(CV_CAP_PROP_FPS, 30);

	if(!webcamL.open(DEVICE_LEFT)){
		cout << "ERROR: Couldn't open LEFT webcam" << endl;
		pthread_exit(NULL);
	}
	webcamL.set(CV_CAP_PROP_FRAME_WIDTH, width);
	webcamL.set(CV_CAP_PROP_FRAME_HEIGHT, height);
	webcamL.set(CV_CAP_PROP_FPS, 30);

	if( !webcamR.isOpened() && !webcamL.isOpened() ){
		cout << "ERROR: WebCams didn't open properly" << endl;
		pthread_exit(NULL);
	}

	//Let cameras focus
	for(int i=0; i<10; i++){get_frame(webcamR); get_frame(webcamL);}
...
//				}
			...
			serialPort.sendArray("mf100", 6);
			usleep(100000);
			serialPort.sendArray("sr7", 6);
			sleep(10);
		}
		//TODO go to zebra
